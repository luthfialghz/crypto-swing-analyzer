                ...
                >
                  ...
                  {expandedCard === coin.coinId && (
                    <div className="border-t border-slate-700/50 p-5 space-y-4 bg-slate-900/30">
                      ...
                      {/* Alternative Coins Section - Show when AI provides alternative coins */}
                      {alternativeCoins.length > 0 && (
                        <div className="mt-4 pt-4 border-t border-slate-700">
                          <h6 className="text-sm font-semibold text-white mb-3 flex items-center gap-2">
                            ...
                            Rekomendasi Koin Alternatif
                          </h6>
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                            {alternativeCoins.map((altCoin: AlternativeCoin, index) => (
                              <div
                                key={`${coin.coinId}-alt-${index}`}
                                className="bg-slate-800/60 rounded-lg p-3 border border-slate-600/30"
                              >
                                <div className="flex items-center gap-2 mb-2">
                                  <img src={altCoin.image} alt={altCoin.symbol} className="w-6 h-6 rounded-full" />
                                  <span className="font-bold text-white text-sm">{altCoin.symbol}</span>
                                </div>
                                <div className="text-xs text-slate-400">H4: {altCoin.h4_change >= 0 ? '+' : ''}{altCoin.h4_change.toFixed(2)}%</div>
                                <div className="text-xs text-slate-400">24h: {altCoin.price_change_percentage_24h >= 0 ? '+' : ''}{altCoin.price_change_percentage_24h.toFixed(2)}%</div>
                                <div className="text-xs text-slate-500 mt-1 italic">{altCoin.reasoning}</div>
                              </div>
                            ))}
                          </div>
                          <p className="text-xs text-slate-500 mt-3">
                            Koin-koin di atas direkomendasikan oleh AI sebagai alternatif investasi yang potensial.
                          </p>
                        </div>
                      )}
                    </div>
